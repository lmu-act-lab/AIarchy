...
class ApproxQComparisonAgent:
    def __init__(self, actions, gamma, alpha, cla_agent) -> None:
        ...
        self.iter = 0
        self.eps = 0.2
        self.weights = Counter(self.get_features(self.actions[0]))


    def get_q_value(self, state, action):
        return self.get_features(action) * self.weights

    def get_features(self, action):
        return Counter(
            self.agent.time_step(
                {}, self.agent.weights, self.agent.sample_num, {action[0]: action[1]}
            ).average_sample
        )

    def get_best_action_value(self, state):
        return max(
            [(action, self.get_q_value(state, action)) for action in self.actions],
            key=lambda x: x[1],
        )
    ...
    def q_value_update(self, reward, prev_state, state, action):
        for weight in self.weights.keys():
            delta = (
                reward
                + (self.gamma * self.get_best_action_value(state)[1])
                - self.get_q_value(prev_state, action)
            )
            self.weights[weight] += (
                self.alpha * delta * self.get_features(action)[weight]
            )
